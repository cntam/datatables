...
$recordsTotalResult = $conn->query($sumSql);
//while($row=mysql_fetch_assoc($recordsTotalResult)){
//	$recordsTotal = $row['sum'];
//}

if ($recordsTotalResult->num_rows > 0) {
    // 输出数据
...
if(strlen($sumSearchSql)>0){
    ...
    $recordsFilteredResult = $conn->query($sumSql.$sumSearchSql);
//    while ($row = mysql_fetch_assoc($recordsFilteredResult)) {
//        $recordsFiltered =  $row['sum'];
//    }
    if ($recordsFilteredResult->num_rows > 0) {
        // 输出数据
